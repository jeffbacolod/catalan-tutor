...
function buildSystemPrompt(inputMethod, nativeLang) {
  const correctionScope = inputMethod === 'voice' ? CORRECTION_VOICE : CORRECTION_TEXT

  const base = nativeLang === 'en'
...
export function useConversation(nativeLang = 'ca') {
  ...
  const sendUserMessage = useCallback(async (text, inputMethod = 'text') => {
    ...
    try {
      const { reply } = await sendMessage(next, buildSystemPrompt(inputMethod, nativeLang))
